package terraform.gcp.security.cloud_pub_sub.google_pubsub_topic_iam_binding.no_public_principals_test

import data.terraform.gcp.security.cloud_pub_sub.google_pubsub_topic_iam_binding.no_public_principals
import rego.v1
...
# Mock: non-compliant resource granting access to allUsers
mock_non_compliant := {
	"planned_values": {"root_module": {"resources": [
		{
			"type": "google_pubsub_topic_iam_binding",
...
test_no_public_principals_message_contains_resource if {
	...
	contains(msg, "Pub/Sub Topic IAM Binding")
}
